...
YOLO_NORMALIZED_BOUNDARY_EPS = 1e-7
...
RNG_FORMAT_VERSION = 1


def normalize_rng_byte_state(value, field_name: str) -> torch.Tensor:
    """Return a validated CPU uint8 RNG state without exposing its values."""
    if isinstance(value, torch.Tensor):
        if value.ndim != 1 or value.numel() == 0:
            raise RuntimeError(f"Invalid {field_name}: type=Tensor dtype={value.dtype} shape={tuple(value.shape)}")
        if value.dtype.is_floating_point or value.dtype.is_complex:
            raise RuntimeError(f"Invalid {field_name}: type=Tensor dtype={value.dtype} shape={tuple(value.shape)}")
        cpu_value = value.detach().cpu()
        if cpu_value.dtype != torch.uint8 and (torch.any(cpu_value < 0) or torch.any(cpu_value > 255)):
            raise RuntimeError(f"Invalid {field_name}: type=Tensor dtype={value.dtype} shape={tuple(value.shape)} values_outside_byte_range")
        return cpu_value.to(dtype=torch.uint8).contiguous().clone()
    if isinstance(value, (bytes, bytearray)):
        if not value:
            raise RuntimeError(f"Invalid {field_name}: type={type(value).__name__} length=0")
        return torch.tensor(list(value), dtype=torch.uint8).contiguous()
    if isinstance(value, np.ndarray):
        if value.dtype != np.uint8 or value.ndim != 1 or value.size == 0:
            raise RuntimeError(f"Invalid {field_name}: type=ndarray dtype={value.dtype} shape={value.shape}")
        return torch.from_numpy(value.copy()).to(dtype=torch.uint8).contiguous()
    if isinstance(value, (list, tuple)):
        if not value or any(isinstance(item, bool) or not isinstance(item, (int, np.integer)) or item < 0 or item > 255 for item in value):
            raise RuntimeError(f"Invalid {field_name}: type={type(value).__name__} length={len(value)} expected_integer_bytes")
        return torch.tensor(list(value), dtype=torch.uint8).contiguous()
    raise RuntimeError(f"Invalid {field_name}: type={type(value).__name__} unsupported_rng_state")


def capture_rng_state() -> dict:
    return {
        "rng_format_version": RNG_FORMAT_VERSION,
        "python_random_state": random.getstate(),
        "numpy_random_state": np.random.get_state(),
        "torch_cpu_rng_state": torch.get_rng_state().detach().cpu().to(dtype=torch.uint8).contiguous().clone(),
        "torch_cuda_rng_states": [state.detach().cpu().to(dtype=torch.uint8).contiguous().clone() for state in torch.cuda.get_rng_state_all()] if torch.cuda.is_available() else [],
    }
...
def restore_rng_state(state: dict) -> None:
    ...
    torch.set_rng_state(normalize_rng_byte_state(state["torch_cpu_rng_state"], "torch_cpu_rng_state"))
    if torch.cuda.is_available() and state.get("torch_cuda_rng_states"):
        torch.cuda.set_rng_state_all([normalize_rng_byte_state(value, f"torch_cuda_rng_states[{index}]") for index, value in enumerate(state["torch_cuda_rng_states"])])
...
def normalize_yolo_box_with_tolerance(
    values: Sequence[float], eps: float = YOLO_NORMALIZED_BOUNDARY_EPS
) -> Tuple[float, float, float, float]:
    """Normalize representation-only boundary noise without accepting invalid boxes."""
    cx, cy, bw, bh = map(float, values)
    if bw <= 0 or bh <= 0:
        raise ValueError("YOLO box has non-positive width or height")
    x1, y1, x2, y2 = cx - bw / 2, cy - bh / 2, cx + bw / 2, cy + bh / 2
    if x1 < -eps or y1 < -eps or x2 > 1.0 + eps or y2 > 1.0 + eps:
        raise ValueError("YOLO box exceeds normalized bounds beyond tolerance")
    x1, y1 = max(x1, 0.0), max(y1, 0.0)
    x2, y2 = min(x2, 1.0), min(y2, 1.0)
    if x2 <= x1 or y2 <= y1:
        raise ValueError("YOLO box has zero area after boundary normalization")
...
def read_yolo_label(label_path: Optional[Path], num_classes: int, image_size: Tuple[int, int], remap_all_to_zero: bool = False) -> List[Tuple[int, float, float, float, float]]:
    ...
    with label_path.open("r", encoding="utf-8", errors="ignore") as f:
        for line_number, line in enumerate(f, 1):
            ...
            try:
                cx, cy, bw, bh = normalize_yolo_box_with_tolerance(vals)
            except ValueError as exc:
                raise RuntimeError(
                    f"Invalid or clipping-required YOLO box: {label_path}:{line_number}: {exc}"
                ) from None
...
def train_torchvision_detector(
    ...
    if bool(train_cfg.get("resume", True)) and resume_source.exists():
        ...
        # Keep serialized CPU RNG tensors on CPU. Moving the full checkpoint to CUDA
        # would turn torch_cpu_rng_state into a CUDA ByteTensor, which set_rng_state rejects.
        resume_checkpoint = torch.load(resume_source, map_location="cpu", weights_only=False)

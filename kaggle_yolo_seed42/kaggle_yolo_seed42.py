"""Official canonical YOLOv8s Seed-42 training on Kaggle GPU."""
...
CONFIG_NAME = "config_runpod_yolo_seed42.yaml"
MODEL = "yolo"
WORK_ROOT = Path("/kaggle/working/marine_yolo_seed42_official")
...
def main() -> None:
    repo = WORK_ROOT / "repository"; run("git", "clone", "--branch", "main", REPOSITORY, str(repo))
    run("git", "-C", str(repo), "merge-base", "--is-ancestor", REQUIRED_ANCESTOR, "HEAD"); run("git", "-C", str(repo), "rev-parse", "HEAD")
    run(sys.executable, "-m", "pip", "install", "-q", "-r", str(repo / "requirements.txt"))
    cfg = yaml.safe_load((repo / CONFIG_NAME).read_text(encoding="utf-8")); cfg["trash_root"] = str(data_root()); cfg["out_dir"] = str(WORK_ROOT / "output")
    runtime = WORK_ROOT / "official_runtime_config.yaml"; runtime.parent.mkdir(parents=True, exist_ok=True); runtime.write_text(yaml.safe_dump(cfg, sort_keys=False), encoding="utf-8")
    ...
    (WORK_ROOT / "OFFICIAL_SEED42_RUN.txt").write_text("model=yolo\nseed=42\nresume=false\nevaluate=false\n", encoding="utf-8")

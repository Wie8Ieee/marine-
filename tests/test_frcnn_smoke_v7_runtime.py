...
from tools.frcnn_smoke_v7_runtime import SmokeExport
...
class V7ExportTests(unittest.TestCase):
    ...
    def assert_failure(self, stage, command):
        with self.assertRaises(RuntimeError): self.export.run_process(stage, command, self.work, self.env)
        self.export.failure(stage, RuntimeError("simulated failure"), 1); self.export.finalize()

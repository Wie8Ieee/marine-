"""Official canonical SSDLite Seed-42 training on Kaggle GPU."""
from __future__ import annotations
import os, subprocess, sys
from pathlib import Path
import yaml

REPOSITORY = "https://github.com/Wie8Ieee/marine-.git"; REQUIRED_ANCESTOR = "70d02255"; CONFIG_NAME = "config_runpod_ssd_seed42.yaml"; MODEL = "ssd"; WORK_ROOT = Path("/kaggle/working/marine_ssd_seed42_official")
...
def main() -> None:
    ...
    env=dict(os.environ,PYTHONDONTWRITEBYTECODE="1"); run(sys.executable,str(repo/"marine_3model_experiment.py"),"--config",str(runtime),"--preflight-only"); subprocess.run([sys.executable,str(repo/"marine_3model_experiment.py"),"--config",str(runtime)],check=True,cwd=repo,env=env); run(sys.executable,str(repo/"tools/verify_training_artifacts.py"),"--model",MODEL,"--out-dir",str(WORK_ROOT/"output"),"--config",str(runtime)); (WORK_ROOT/"OFFICIAL_SEED42_RUN.txt").write_text("model=ssd\nseed=42\nresume=false\nevaluate=false\n",encoding="utf-8")
